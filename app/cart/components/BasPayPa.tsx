...
function BasPayPa({ cena, data }: {
    ...
    const onApprove = async (datqa: any, actions: any) => {
        try {
            const da = await axios.post(`/api/${datqa.orderID}/capturePayment`, { data, constKorpa },);
        } catch (error) {
            console.error(error);
        }
    };
    return (
        <PayPalButtons onClick={() => setRefreshKey(refreshKey + 1)} forceReRender={[cena, data]} className="w-full sticky " style={{
